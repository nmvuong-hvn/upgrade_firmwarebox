...
class DownloadingManagerAndroid(val context: Context) : NetworkConnectionManager.NetworkStateListener{
    ...
    private val downloadBroadcast = object : BroadcastReceiver(){
        override fun onReceive(p0: Context?, p1: Intent?) {
            if (p1?.action == DownloadManager.ACTION_DOWNLOAD_COMPLETE) {
                scope.launch {
                    val referenceId: Long = p1.getLongExtra(DownloadManager.EXTRA_DOWNLOAD_ID, -1)
                    try {
                        withContext(Dispatchers.Main.immediate){
                            Toast.makeText(p0, "Downloaded successfully", Toast.LENGTH_SHORT).show()
                        }
                        DownloadDatabase.getInstance().downloadDao().updateState(referenceId, DownloadEntity.STATUS_COMPLETED)
                        Log.d(TAG, "onReceive: -----> referenceId = $referenceId")
                    }catch (e : Exception){
                        Log.d(TAG, "onReceive: ====> e = ${e.message}")
                    }
                }
            }
        }
    ...
    companion object {
        ...
        var downloadingManager : DownloadingManagerAndroid? = null
        fun create(context: Context) {
            downloadingManager = DownloadingManagerAndroid(context)
        }
        fun getInstance() : DownloadingManagerAndroid {
    ...
    override fun onNetworkAvailable() {
        scope.launch {
            ...
            downloadingList.forEach {
                ...
                if (it.status == DownloadEntity.STATUS_FAILED){
                    DownloadDatabase.getInstance().downloadDao().deletedById(it.downloadId)
                }
